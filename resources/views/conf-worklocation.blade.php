...
<div class="row">
    ...
    <ul class="nav nav-tabs">
        ...
        <li class="nav-item">
            <a class="nav-link active" href="{{ asset('conf-worklocation') }}">勤務地設定</a>
        </li>
        <li class="nav-item">
    ...
    <div class="col-md-12">
        <table class="table mt-3">
            ...
            <tbody>
                ...
                <tr class="line_{{ $work_location->status }}">
                    <td style="display:none;">{{ $work_location->id }}</td>
                    <td>
                        @if( $work_location['status'] === 0)
                            <p class="text-success">有効</p>
                        @else
                            <p class="text-muted">無効</p>
                        @endif
                    </td>
                    <td>{{ $work_location->display_order }}</td>
                    <td>{{ $work_location->name }}</td>
                    <td>{{ $work_location->created_at->format('Y年m月d日') }}</td>
                    <td>
                        <button type="button" class="btn-original" data-bs-toggle="modal" data-bs-target="#updateModal"  data-bs-id="{{ $work_location['id'] }}" data-bs-status="{{$work_location['status']}}" data-bs-name="{{$work_location['name']}}" data-bs-order="{{$work_location['display_order']}}"><i class="far fa-edit mr-3"></i></button>
                    </td>
                </tr>
                @endforeach
            </tbody>
